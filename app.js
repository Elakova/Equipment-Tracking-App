...
// ── Checked-out state ───────────────────────────────
function getCheckedOutEquipment() {
  // Walk the log (newest first) and record the first-seen status per item
  const seen = new Set();
  const out  = new Set();
  for (const entry of log) {
    if (!seen.has(entry.equipment)) {
      seen.add(entry.equipment);
      if (entry.status === 'OUT') out.add(entry.equipment);
    }
  }
  return out;
}
...
function renderPicker(list, query = '') {
  ...
  eqPickerGrid.innerHTML = filtered.map(e => {
    const isOut = checkedOut.has(e.name);
    return `
      <button class="picker-chip${isOut ? ' picker-chip--out' : ''}"
              data-name="${escHtml(e.name)}"
              aria-label="${isOut ? 'Currently out: ' : 'Add '}${escHtml(e.name)}"
        ...
        ${isOut ? '<span class="picker-chip__out-badge">OUT</span>' : ''}
      </button>
    `;
  }).join('');
  ...
  eqPickerGrid.querySelectorAll('.picker-chip').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      if (checkedOut.has(name)) {
        showToast(`⛔ ${name} is currently OUT — wait for it to be returned`, 'red');
        return;
      }
      addToQueue(name);
...
function addToQueue(name) {
  ...
  // Block if currently checked out
  const checkedOut = getCheckedOutEquipment();
  if (checkedOut.has(name)) {
    showToast(`⛔ ${name} is currently OUT — wait for it to be returned`, 'red');
    return;
...
function logEntry(status) {
  ...
  // Refresh picker to reflect new OUT/IN states
